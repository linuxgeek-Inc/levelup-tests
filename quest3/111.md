...
**11-3) 본 영상 다운로드**
```bash
wget https://dl.cv.ethz.ch/bdd100k/data/100k_images_train.zip -O /workspace/100k_images_val.zip
```
...
**13-2) 현재 디렉토리에서 오랫동안 사용 하지 않은 파일들을 확인.**
* 오래 사용하지 않은 파일을 찾아서 최근 접근 시간 순으로 정렬해 상위 10개를 보여줍니다.
...
**14-1) 샘플 영상 파일 삭제**
```bash
rm /workspace/sample_video_1GB.zip
```
...
**14-2) 본 영상 파일 다운로드 재시도**
* -c : 중단된 파일부터 다운로드를 재개.
```bash
wget -c https://dl.cv.ethz.ch/bdd100k/data/100k_images_train.zip -O /workspace/100k_images_val.zip
```
